...
# ══════════════════════════════════════════════════════════════════
SIEM_SERVER = "ws://192.168.56.1:8765"  # ← ضع IP الجهاز اللي يشتغل عليه AKD
...
RULES = [
    ...
    {"id": "R-014", "pattern": r"(wget|curl).*(http).*-O.*(/tmp|/var)", "severity": "critical",
     "name": "Suspicious Download"},
    {"id": "R-015", "pattern": r"chmod.*777|chmod.*\+x.*/tmp", "severity": "high", "name": "Suspicious chmod"},
    # تمت إضافة قاعدة هجمات الويب وحقن قواعد البيانات هنا ↓
    {"id": "R-016", "pattern": r"(?i)(sqlmap|union.*select|select.*from|%27|')", "severity": "critical",
    ...
    def __init__(self):
        if OS_TYPE == "Linux":
            candidates = [
                ...
                "/var/log/nginx/access.log",  # تمت إضافة مسار Nginx Web Logs
                "/var/log/apache2/error.log",
                "/var/log/apache2/access.log",  # تمت إضافة مسار Apache Web Logs
...
async def run():
    ...
    print(f"\n{'=' * 55}")
    print(f"  SecureWatch Agent — {AGENT_ID}")
        ...
        try:
            ...
            async with websockets.connect(SIEM_SERVER, ping_interval=20, ping_timeout=10) as ws:
                ...
                while True:
                    ...
                    # فحص المنافذ Port Scan (تمت إعادته)
